# ADB_APP — adb_kit + ADB Vision

This workspace contains:

| Folder | Description |
|---|---|
| [`adb_kit/`](adb_kit/) | A typed Dart package that wraps every major Android Debug Bridge command — devices, packages, input, screen, files, logcat, intents, settings, dumpsys, scripts, etc. Pure Dart, unit-tested, no Flutter dependency. |
| [`adb_vision/`](adb_vision/) | A Flutter desktop example app (macOS / Windows / Linux) that uses `adb_kit` to provide a complete GUI for ADB — live mirror, input injection, package manager, file manager, logcat, intent composer, scripting/recording. |
...
cd adb_vision && flutter pub get && flutter run -d macos
```
...
- ⏭️ scrcpy H.264 mirror backend — needs a native decoder plugin; the screencap fallback runs at 1–15 fps.
- ⏭️ View-hierarchy inspector / OCR / pixel-RGB asserts — `ScriptPlayer._assert` is the integration point.
- ⏭️ CI workflow / installers / i18n.
